...
## Introduction
  As a data scientist for a marketing company, a new client has recently accquired a mall in Saint Louis, Missouri. The owner has contacted us to help identify different groups among the customer demographic. The owner wants to understand the customers, so the segments can be given to the marketing team for planning the strategy accurately. The only data we have is from a previous owner, were they collected certain metrics about the customers.
